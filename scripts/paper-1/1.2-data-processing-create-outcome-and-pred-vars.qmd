...
system.time(
  df_psu_tmax_tmin_temp_vars_added <- df_psu_tmax_tmin[, hotday_35 := ifelse(max_temp > 35, 1, 0)
        ...
        , long_term_avg := mean(max_temp, na.rm = T), by = psu
      ][
        , hotday_long_avg := ifelse(max_temp > long_term_avg+5, 1, 0)
      ][
        ...
        , consec_days_lt_avg := ifelse(hotday_long_avg == 1, 1:.N, 0L), by = rleid(hotday_long_avg == 1)
      ][
...
beep(sound = 3)
head(df_psu_tmax_tmin_temp_vars_added, 1000)
```
...
## Save work
```{r}
save(df_psu_tmax_tmin_2022, file = "./bigdata/images-paper-1/1.2-b-dhs-psu-all-temp-vars-created-daily-2022.RData")
beep(sound = 3)
names(df_psu_tmax_tmin_2022)
```

# aggregate data at PSU level

## first create variables for consecutive days
```{r}
df_psu_tmax_tmin_2022_consec <- df_psu_tmax_tmin_2022[
  , num_consec_hot_nights_2 := ifelse(consec_nights_20 > 2, 1, 0), by = psu
][
  , num_consec_hot_days_35_3 := ifelse(consec_days_35 > 2, 1, 0), by = psu
][
  , num_consec_hot_days_35_4 := ifelse(consec_days_35 > 3, 1, 0), by = psu
][
  , num_consec_hot_days_35_5 := ifelse(consec_days_35 > 4, 1, 0), by = psu
][
  , num_consec_long_avg_tmax_5 := ifelse(consec_days_lt_avg > 4, 1, 0), by = psu
]
names(df_psu_tmax_tmin_2022_consec)
```
## Now aggregate
```{r}
df_psu_tmax_tmin_2022_psu <- df_psu_tmax_tmin_2022_consec[,
                  .(lat = first(lat),
                    long = first(long),
                    max_temp_avg = mean(max_temp),
                    min_temp_avg = mean(min_temp),
                    tmax_long_term_avg = mean(long_term_avg),
                    num_hotday_long_avg = sum(hotday_long_avg),
                    num_hotnight_20 = sum(hotnight_20),
                    num_hotday_35 = sum(hotday_35),
                    num_hotday_95 = sum(hotday_95),
                    num_hotday_35_or_95 = sum(hotday_35_95),
                    num_hotday_35_and_95 = sum(hotday_both),
                    num_consec_hotdays_35_3 = sum(num_consec_hot_days_35_3),
                    num_consec_hotdays_35_4 = sum(num_consec_hot_days_35_4),
                    num_consec_hotdays_35_5 = sum(num_consec_hot_days_35_5),
                    num_consec_long_avg_tmax_5 = sum(num_consec_long_avg_tmax_5)), 
                  by = psu]
